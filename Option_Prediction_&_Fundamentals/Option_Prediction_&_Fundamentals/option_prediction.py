...
def option_prediction (Company_Names, option_dates, output_folder=OUTPUT_FOLDER, export_table=True):
    ...
    for x in Company_Names:
        ...
        tickerp = yf.download(x, start= datetime.date.today() - datetime.timedelta(days=3))
        ...
        for y in option_dates:
            ...
            CPSP = ((Calls_WOI*0.6)+ (Puts_WOI*0.4))
            CP_WMid_list.append(CPSP)

            for b in ticker_t_p_list:
                CP_WMid_Price_Diff = CPSP - b
    ...
    Option_Analysis_T0 = pd.DataFrame({'Tickers': C_N_df, "Todays Price": Prices_df,'Option Exp Date': option_dates_df,                                     "Days_to_Exp": Days_to_Exp,"Calls_WOI": Calls_WOI_list,                                     "Puts_WOI": Puts_WOI_list, "CPSP":CP_WMid_list})
    Option_Analysis_T0["implied Change from T0"] = Option_Analysis_T0["CPSP"] - Option_Analysis_T0["Todays Price"]
    ...
    for x in Company_Names:
        ...
        Pct_Change_T1 = list(Price_diff_T1_l / Option_Analysis_T1_reset["CPSP"][:3]*100)
    ...
    Option_Analysis_Weekly = pd.DataFrame({'Tickers': C_N_df, "Todays Price": Prices_df,                                           'Option Exp Date': option_dates_df,                                            "Days_to_Exp": Days_to_Exp,"Calls_WOI": Calls_WOI_list,                                            "Puts_WOI": Puts_WOI_list, "CPSP":CP_WMid_list,                                           "implied Change to prior week":Price_diff_T1_l_df,                                           "implied %Change to prior week": Pct_Change_T1_df})
