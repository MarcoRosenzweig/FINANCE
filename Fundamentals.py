...
def fundamentals (Company_Names, output_folder=OUTPUT_FOLDER, export_table=True):
    ...
    for x in Company_Names:
        ...
        data = yf.download(x, start= todaydate - datetime.timedelta(days=50))
        data_c = data["Close"]
        data_beg = data_c[-7]
        data_end = data_c[-1]
        data_ma_prices = np.sum(data_c)
        data_ma_days = np.shape(data_c)
        #Calculate Return
    ...
    Fundamentals_X_input = pd.DataFrame({"Tickers":Company_Names,"MarketCap":C_N_MarketCap,"Beta":C_N_Beta, "50 Day Moving Average": moving_averages_list,"MA to TP %":price_vs_ma_list, "1W %Change":year_pct_change_list, "30day Volatility":data_vol_list})
